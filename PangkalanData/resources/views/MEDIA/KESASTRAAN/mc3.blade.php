...
@include('PARTIAL.MenuMedia')
    ...
    <div class="judul">
        <th>DOKUMEN DATA MUSIKALISASI PUISI NASIONAL</th>
    </div>
<!-- 
                        ...
                        @auth
                            @if (Auth::user()->level != 'tamu')
                                ...
                                <div style="margin:5px auto">
                                    <form method="POST" id="media_form" role="form" action="{{ url('/media/kesastraan/musikalisasi_puisi_nasional/' . $a->id)}}" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                ...
                                @else
                                    ...
                                    <a id="hapus_media" href="{{ url('/media/kesastraan/musikalisasi_puisi_nasional/hapus/' . $a->id) }}" style="margin-left:12px; color:white" class="btn btn-sm btn-danger">Hapus Dokumen</a>
                                @endif
                            @else
                                @if ($a->media == "")
                                <div style="margin:5px auto">
